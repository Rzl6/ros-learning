...
int main(int argc, char ** argv)
{
  ...
  // 1. 先检查用户有没有传参数进来
  // argc < 2 意味着用户只输了命令，没输数字
  if (argc < 2) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "用法: ros2 run 包名 节点名 <数字>");
    return 1;
  }

  auto action_client = std::make_shared<MinimalActionClient>();

  // 2. 使用 atoi 函数把字符串参数 (argv[1]) 转成整数
  int64_t target_num = atoi(argv[1]);
  
  // 3. 把这个动态的数字传进去
  action_client->send_goal(target_num);
